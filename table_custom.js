// -----------------------------------------------------------------------------
// Grid-look Heatmap (Explore-compatible filtering via Highcharts point clicks)
// - Single heatmap series (every cell is a Highcharts point)
// - Grouped headers (HTML) above chart, centered labels
// - Optional sorting by clicking leaf headers
// - Click-to-filter: emits Explore legacy array-of-objects from viz scope
// -----------------------------------------------------------------------------
looker.plugins.visualizations.add({
  ...
  create (element) {
    ...
    // Scroller + inner (for horizontal scroll)
    ...
    scroller.addEventListener('scroll', () => {
      // keep headers’ scrollLeft in sync
  ...
  async updateAsync (data, element, config, queryResponse, details, done) {
    ...
    const viz = this; // <-- capture viz scope for filter emits (critical for Explore)
    ...
    resolvedCols.forEach((col, idx) => {
      ...
      if (config.enable_sorting) {
        leaf.addEventListener('click', () => {
          if (!viz._sortState || viz._sortState.key !== col.key) viz._sortState = { key: col.key, dir: 'asc' };
          else viz._sortState.dir = (viz._sortState.dir === 'asc') ? 'desc' : 'asc';
          viz.updateAsync(data, element, config, queryResponse, details, done);
        });
      }
    });

    // Build a single heatmap series with all cells
    ...
    for (let y=0; y<yCount; y++) {
      for (let x=0; x<xCount; x++) {
        ...
        // numeric parse (for future heat), fallback to 0
          ...
          point: {
            events: {
              click: function () {
                ...
                const colKey = this.custom?.colKey;
                if (colKey !== clickKey) return; // only the configured column is clickable
                const raw = this.custom?.raw;
                const fmt = this.custom?.fmt ?? (raw == null ? '' : String(raw));
                if (raw == null || raw === '') return;

                // *** CRITICAL: emit from viz scope so Explore hears it ***
    ...
    const applySortIndicators = () => {
      if (!config.enable_sorting || !viz._sortState?.key) return;
      Array.from(viz.leafRow.children).forEach(el => {
        const base = (el.textContent || '').replace(/\s*[▲▼]$/, '');
        el.textContent = base;
      });
      const idx = resolvedCols.findIndex(c => c.key === viz._sortState.key);
      if (idx >= 0) {
        const el = viz.leafRow.children[idx];
        const tri = (viz._sortState.dir === 'asc') ? ' ▲' : ' ▼';
        el.textContent = (el.textContent || '') + tri;
      }
    };
    applySortIndicators();

    this._redrawHeaders = () => { /* widths are fixed from colWidths */ };
