...
looker.plugins.visualizations.add({
    ...
    options: {
        ...
        header_info_icon_columns: {
            label: "Header Info Icon Columns (col index array)",
            type: "string",
            default: "[]",
            section: "Tooltips",
        },
        header_info_icon_tooltips_json: {
            label: 'Header Info Icon Tooltips JSON (e.g. {"2":"Description"})',
            type: "string",
            default: "{}",
            section: "Tooltips",
        },
    ...
    `;
        ...
        #simple_grid_container .sg-header-with-addon{
          display:flex;
          align-items:center;
          justify-content:center;
          gap:6px;
        }
        #simple_grid_container .sg-header-info-icon{
          width:14px;
          height:14px;
          border-radius:999px;
        ...
        /* pagination */
        #simple_grid_container .sg-pagination{
          display:flex;
          justify-content:center;
          align-items:center;
          gap:8px;
        ...
        #simple_grid_container .sg-page-btn{
          min-width:50px;
          height:25px;
          padding:0 18px;
          border:2px solid #111a44;
        ...
        #simple_grid_container .sg-page-ellipsis{
          min-width:24px;
          height:25px;
          display:inline-flex;
          align-items:center;
    ...
    _showTooltip(text, x, y) {
        ...
        this._tooltipEl.innerHTML = this._sanitizeTooltipHTML(text);
    ...
    _buildPaginationHTML(currentPage, totalPages) {
        ...
        const items = this._buildPaginationItems(currentPage, totalPages);

        let html = `<div class="sg-pagination">`;

        const prevDisabled = currentPage <= 1 ? " sg-disabled" : "";
        html += `<div class="sg-page-btn${prevDisabled}" data-page-action="prev"><</div>`;

        items.forEach((item) => {
            if (item === "...") {
                html += `<div class="sg-page-ellipsis">...</div>`;
                return;
            }

            const activeClass = item === currentPage ? " sg-active" : "";
            html += `<div class="sg-page-btn${activeClass}" data-page="${item}">${item}</div>`;
        });

        const nextDisabled = currentPage >= totalPages ? " sg-disabled" : "";
        html += `<div class="sg-page-btn${nextDisabled}" data-page-action="next">></div>`;
    ...
    async updateAsync(data, element, config, queryResponse, details, done) {
        ...
        const headerInfoIconIndexSet = this._parseJsonIndexSet(config.header_info_icon_columns || "[]");
        ...
        const headerInfoIconTooltipsObj = this._parseJsonObject(config.header_info_icon_tooltips_json || "{}");
        ...
        const totalRows = rowsWithIndex.length;
        ...
        const pagedRowsWithIndex = enablePagination
            ? rowsWithIndex.slice((this._currentPage - 1) * rowsPerPage, this._currentPage * rowsPerPage)
            : rowsWithIndex;
        ...
        visibleFields.forEach((f, colIndex) => {
            ...
            let headerInner = this._escapeHTML(label);

            if (headerInfoIconIndexSet.has(colIndex)) {
                const tip =
                    headerInfoIconTooltipsObj[String(colIndex)] !== undefined
                        ? headerInfoIconTooltipsObj[String(colIndex)]
                        : headerInfoIconTooltipsObj[f.name];

                const tipText = typeof tip === "string" ? tip : "";
                const tipAttr = tipText ? ` data-sg-tip="${this._escapeHTML(tipText)}"` : "";

                headerInner = `
          <div class="sg-header-with-addon">
            <span>${this._escapeHTML(label)}</span>
            <span class="sg-header-info-icon"${tipAttr}>i</span>
          </div>`;
            }
        ...
        pagedRowsWithIndex.forEach(({ row, originalIndex }, pageDisplayIndex) => {
            const isSelected = originalIndex === this._selectedRowIndex;
            const rowClass = isSelected ? " class=\"sg-selected\"" : "";
            html += `<tr${rowClass}>`;

            if (showRowNumbers) {
                const overallDisplayIndex = enablePagination
                    ? ((this._currentPage - 1) * rowsPerPage) + pageDisplayIndex + 1
                    : pageDisplayIndex + 1;
        ...
        if (enablePagination) {
            html += this._buildPaginationHTML(this._currentPage, totalPages);
        }
        ...
        container.onclick = function (evt) {
            ...
            const headerInfoIcon = evt.target.closest(".sg-header-info-icon");
            const cell = evt.target.closest("td");
            const pageBtn = evt.target.closest(".sg-page-btn");

            if (pageBtn) {
                if (pageBtn.classList.contains("sg-disabled")) return;

                const pageAttr = pageBtn.getAttribute("data-page");
                const actionAttr = pageBtn.getAttribute("data-page-action");

                if (pageAttr) {
                    const pageNum = Number(pageAttr);
                    if (Number.isInteger(pageNum) && pageNum >= 1) {
                        viz._currentPage = pageNum;
                        viz._forceRerender();
                    }
                    return;
                }

                if (actionAttr === "prev" && viz._currentPage > 1) {
                    viz._currentPage -= 1;
                    viz._forceRerender();
                    return;
                }

                if (actionAttr === "next" && viz._currentPage < totalPages) {
                    viz._currentPage += 1;
                    viz._forceRerender();
                    return;
                }
